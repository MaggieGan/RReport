  ...
  for(j in 1:Cnt_sub){
    ...
    if(dim(TLnow)[1]>0){
		...
		if(!is.na(V_Frate)){
			...
			TMP=TMP[,-3]
			NameNow=names(TMP)
			if(ALL){NameNow[1:3]=c("col1","col2","Overall")}
			else{	NameNow[1:2]=c("col1","col2")}
			names(TMP)=NameNow
		}
		else {
			TMP=tabfreq(x =TLnow$vgroup,y=TLnow$Vrate,ylevels=Vrate_levels,yname="HQMS",xlevels=group1_levels,
...
# print(dim(TLin))
 ...
 widthTmp[NotecolID]=widthTmp[NotecolID]*(6**(1/widthTmp[NotecolID]))
  ...
  if(nameCOl>1){
   ...
   # 除最后一个外都需要划分界线
    ...
    for(k in 1:(dim(AddlineNote)[1])){
	  ...
	  if(NoteNow!=lastNote){
    Indexmax=max(which(TLin[,1]==NoteNow,arr.ind=T))
...
MyFTable=setFlexTableWidths( MyFTable,widths = c(widthTmp/sum(widthTmp)*6.8))
 # if(dim2==2){MyFTable=setFlexTableWidths( MyFTable,widths = c(1,2,2,1.8))}
 # else if(dim2==3){ MyFTable=setFlexTableWidths( MyFTable,widths = c(0.3,1.5,2,2,1))}
 # else if (dim2==4){
 #	if(widthTmp[1]<=10){MyFTable=setFlexTableWidths( MyFTable,widths = c(1.2,1.8,1.8,1.8,0.2)) }
 #	else{MyFTable=setFlexTableWidths( MyFTable,widths = c(2.3,1.4,1.4,1.4,0.2)) }
#	}
 # else if (dim2==5 & widthSum<85){
 # 	if(widthTmp[1]<=20){MyFTable=setFlexTableWidths( MyFTable,widths = c(1.2,1.35,1.35,1.35,1.35,0.2)) }
#	else{  MyFTable=setFlexTableWidths( MyFTable,widths = c(1.56,1.26,1.26,1.26,1.26,0.2)) }
#	}
# else if (widthSum<=85){MyFTable=setFlexTableWidths( MyFTable,widths = c(widthTmp/sum(widthTmp)*6.8))}

 # if(dim2>3 & dim2<=4){
#    MyFTable[,(1:nameCOl)+1] = parProperties(text.align = "left")
 #   MyFTable[,((nameCOl+1):dim2)+1] = parProperties(text.align = "right")
 # }
#  else if (dim2>4){
#  MyFTable[,1:nameCOl] = parProperties(text.align = "left")
#    MyFTable[,(nameCOl+1):dim2] = parProperties(text.align = "right")
#  }
#  else{
...
# widthTmp=widthTmp/sum(widthTmp)
#print(sum(widthTmp))
#   MyFTable = setFlexTableWidths( MyFTable,widths=6.8*widthTmp)
  return(MyFTable)
